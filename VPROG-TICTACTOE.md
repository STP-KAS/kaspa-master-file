...
L1 follow / prove / settle live in [kaspanet/vprogs](https://github.com/kaspanet/vprogs), not in this repo. `ttd` is a runner wrapper plus DA HTTP. **Do not weld** this onto “vProgs live.” Host pin tracks [vprogs#152](https://github.com/kaspanet/vprogs/pull/152) **draft** `reorg-safety#74e24551` (stacked on [#148](https://github.com/kaspanet/vprogs/pull/148)). Guest ELF still fork `bridge-live-lane#128dd05f`.

[`a758a9b`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/a758a9b) documents a **private multi-machine tn10 CUDA run**. **Not** a public product. [`36ccdbc`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/36ccdbc) dropped the runbook “reorg panics the prover” limit; [`92146b4`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/92146b4) rides [#152](https://github.com/kaspanet/vprogs/pull/152) draft head. GPU-abort on rollback still TODO in #152. `.cargo/config.toml` still defaults `RISC0_DEV_MODE=1`. Guest ELF gitignored (`just build-guest`).
...
1. **Guest pin ≠ host pin.** At tip `92146b4`, host `Cargo.lock` is kaspanet `reorg-safety#74e24551` (= #152 draft head). An earlier same-day note said host `settle-resume#da2a7f26`; that lock moved. Guest is workspace-excluded; `guest/Cargo.lock` still biryukovmaxim `bridge-live-lane#128dd05f` ([`a9a44da`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/a9a44da)). Host `[patch]` does not apply to `just build-guest`. Pin the ELF to the same rev, or document a frozen ABI.
...
6. **Aggregate-prover panic is now [vprogs#152](https://github.com/kaspanet/vprogs/pull/152) draft.** Host pin `92146b4` → `reorg-safety#74e24551` (#152 head). Load-bearing commit [`3d106c2`](https://github.com/kaspanet/vprogs/commit/3d106c2471c23641923ed7b384818ca3408cd6ef). PR out of scope: in-flight GPU abort on rollback. Guest ELF pin did not move. `abi.rs:22` expect still present. `.cargo/config.toml` still defaults `RISC0_DEV_MODE=1`.
...
| [kaspanet/vprogs#152](https://github.com/kaspanet/vprogs/pull/152) `reorg-safety` | **Draft**, base `settle-resume`. Head `74e24551` (CI simnet step). P0.1 interior batch-id hole; P0.2 dead lane-proof defer. GPU-abort on rollback still TODO. Tictactoe host pin `92146b4`. PR body still says tt pins `ac67c5f9` (one behind). |
| [kaspanet/vprogs#148](https://github.com/kaspanet/vprogs/pull/148) `settle-resume` | Draft. Parent of `reorg-safety`. 60 s VCC livelock = dust-storm note in the tn10 runbook. First consumer **moved off** this head. |
