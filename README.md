...
| [biryukovmaxim/vprog-tictactoe](https://github.com/biryukovmaxim/vprog-tictactoe) | **Guest demo, not a public product.** 97 commits, tip `522280d`. Host pins **`reorg-safety#ac67c5f9`** (**no open PR**, 5 ahead of [#148](https://github.com/kaspanet/vprogs/pull/148)). Guest ELF still fork `bridge-live-lane#128dd05f` — `just check` now actually fails on guest clippy/`+risc0`, but that gate is on the old battery. tn10 CUDA runbook `a758a9b`. [#24](https://github.com/biryukovmaxim/vprog-tictactoe/issues/24). |
