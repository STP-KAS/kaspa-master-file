...
L1 follow / prove / settle live in [kaspanet/vprogs](https://github.com/kaspanet/vprogs), not in this repo. `ttd` is a runner wrapper plus DA HTTP. **Do not weld** this onto “vProgs live.” Host pin tracks vprogs branch **`reorg-safety#ac67c5f9`** (5 commits on top of [#148](https://github.com/kaspanet/vprogs/pull/148) draft `da2a7f26`). **No open PR** for `reorg-safety` as of 21 Sep. Guest ELF still fork `bridge-live-lane#128dd05f`.

[`a758a9b`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/a758a9b) documents a **private multi-machine tn10 CUDA run** (fresh covenant, full match, real proofs, claims). **Not** a public product. [`36ccdbc`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/36ccdbc) then dropped the runbook “reorg panics the aggregate prover” limit by pinning `reorg-safety#ac67c5f9` (defer dead lane-proof: vprogs [`3d106c2`](https://github.com/kaspanet/vprogs/commit/3d106c2471c23641923ed7b384818ca3408cd6ef)). That branch has **no open PR**. `.cargo/config.toml` still defaults `RISC0_DEV_MODE=1`. Dust-storm / 60 s wRPC = #148. Guest ELF gitignored (`just build-guest`).
...
6. **Aggregate-prover panic claimed closed by pin `36ccdbc`.** Host now `reorg-safety#ac67c5f9`. The load-bearing vprogs commit is [`3d106c2`](https://github.com/kaspanet/vprogs/commit/3d106c2471c23641923ed7b384818ca3408cd6ef) (defer dead lane-proof; test `dead_lane_proof.rs`). Runbook `Current limits` section **removed**. **No open PR** for `reorg-safety`. Guest ELF pin did not move. `abi.rs:22` expect still present on that branch. `.cargo/config.toml` still defaults `RISC0_DEV_MODE=1`.
...
| [kaspanet/vprogs](https://github.com/kaspanet/vprogs) `reorg-safety#ac67c5f9` | **No open PR.** 5 ahead of #148: defer dead lane-proof ([`3d106c2`](https://github.com/kaspanet/vprogs/commit/3d106c2471c23641923ed7b384818ca3408cd6ef)), restore batch log across interior id gaps. Tictactoe host pin as of `36ccdbc`. Desk notes: [36ccdbc](https://github.com/biryukovmaxim/vprog-tictactoe/commit/36ccdbcf11df9473e6d35e9b6d53d36f7b30e1bc#commitcomment-201315246) · [3d106c2](https://github.com/kaspanet/vprogs/commit/3d106c2471c23641923ed7b384818ca3408cd6ef#commitcomment-201315248). |
