...
| vProgs | Master still [`f9b84a8`](https://github.com/kaspanet/vprogs/commit/f9b84a863a7c7c20586a9cf947550475e894f72e) (28 Jul). No release. **22 Sep ~08:32Z:** [#148](https://github.com/kaspanet/vprogs/pull/148) **closed unmerged** 07:30Z (closed head `78501881`; the old draft head was `da2a7f26`). Resume work is [#154](https://github.com/kaspanet/vprogs/pull/154), **open, not draft**, head `f84525b5`, 25 commits, base [#153](https://github.com/kaspanet/vprogs/pull/153). [#153](https://github.com/kaspanet/vprogs/pull/153) is open, not draft, one commit `3496a8c5` (cancel the lane-proof fetch on shutdown), base #147. [#152](https://github.com/kaspanet/vprogs/pull/152) is still **draft**: closed 07:29Z, reopened 07:30Z, head **`59b30920`** (`ci: drop the runner simnet suite step`). Parent of that commit is `0636cff1`. `74e24551...59b30920` **diverged**, then the demo lock caught up at 09:14Z (see tictactoe). Stack: #147 `74ee1b7d` → #153 → #154 → #152. Unchanged this morning: #149 `51b56aab`, #131 `004a5948`, #134 `add37bed`, #136 `d2da8ad0`, #140 `28f0aee9`. No other kaspanet PR updated on 22 Sep. Research only. Desk comment [5774935036](https://github.com/kaspanet/vprogs/pull/152#issuecomment-5774935036): `reform_committed_gap` returns 0 on `LaneProofFailed` and does not run again, so a timeout is treated as a dead block for the life of the process. |
| tictactoe | Tip [`4f27dd1a`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/4f27dd1a20ce) (22 Sep 09:14Z): `pin vprogs reorg-safety 59b30920`. **Cargo.lock read:** host `reorg-safety#59b30920`, which is #152 HEAD. `74e24551` is gone from that lock. Rusty still `eb0a856`. #152’s body still says the app pins `0636cff1` (the parent commit). **guest/Cargo.lock** was not in this commit; it is still the fork `bridge-live-lane#128dd05f`. Not a product testnet. |
...
| rusty / DK / Argent / SS / vProgs | Tips hold: rusty `eb0a856`, DK `ad45e24`, Argent `e76ee07`, SS `3ed9733`, vprogs `f9b84a8`. | No tip/tag/merge/Final. |
...
| vprogs | https://github.com/kaspanet/vprogs | Provable computation. Research. No public testnet as product. Master `f9b84a8` (28 Jul). **22 Sep:** [#148](https://github.com/kaspanet/vprogs/pull/148) closed unmerged. Stack #147 → [#153](https://github.com/kaspanet/vprogs/pull/153) → [#154](https://github.com/kaspanet/vprogs/pull/154) open `f84525b5` → [#152](https://github.com/kaspanet/vprogs/pull/152) draft `59b30920`. Guest-demo lock caught up to `59b30920` at 09:14Z. |
