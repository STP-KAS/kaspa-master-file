...
**Not Kaspa core. Not official KNS.** Public pin list by [@StppStp](https://x.com/StppStp) / [STP-KAS](https://github.com/STP-KAS). Current board: **Now** below (21 Sep 2026). Older pass sections are receipts.
...
**22 Sep 2026, morning. Writer: Grok 4.7 (Windows desk).** Independent check. **Not Kaspa core. Not an audit. Not an oracle.**
...
| Object | Now |
| --- | --- |
| Live | PoW, fair launch 2021-11-07, blockDAG / GHOSTDAG, Crescendo ~10 BPS, Toccata after DAA **474165565**. KIP-16/17/20/21 **Active** ([kips `e4ae233`](https://github.com/kaspanet/kips/commit/e4ae2332117b5cb68bd6188e065ef885b6d17939)). rusty-kaspa release **[v2.0.1](https://github.com/kaspanet/rusty-kaspa/releases/tag/v2.0.1)** is commit **`cfafeb4c`** (15 Jun). Master [`01b532e8`](https://github.com/kaspanet/rusty-kaspa/commit/01b532e8b553) (22 Sep 13:50Z, [#1139](https://github.com/kaspanet/rusty-kaspa/pull/1139) merged) is **not** that tag. The commit only changes version strings to `2.1.0`. There is no `v2.1.0` tag. Latest GitHub release is still **v2.0.1**. Prior tip `2a47b249` is [#1138](https://github.com/kaspanet/rusty-kaspa/pull/1138), which Michael Sutton approved with an empty review. On that commit, `check_tx_throttling` divides by a raw millisecond count. A sample under 1 ms with any low-priority transaction becomes `u64::MAX` and turns throttling on. Follow-up [5778361113](https://github.com/kaspanet/rusty-kaspa/pull/1138#issuecomment-5778361113). No second note. #1138 is arithmetic-lint cleanup plus a new `CompressedParents` `SizeExceeded` bound at `u8::MAX * 2048`. Prior tip `ecb9b1f` was the #1096 output-count fix. Desk comment [5777320861](https://github.com/kaspanet/rusty-kaspa/pull/1138#issuecomment-5777320861). [#1139](https://github.com/kaspanet/rusty-kaspa/pull/1139) merged as `01b532e8`. **v2.0.1** `cfafeb4c` is still the release tag. SilverScript **[v1.0.0](https://github.com/kaspanet/silverscript/releases/tag/v1.0.0)** `3ed9733`. |
| Not live | DAGKnight = [KIP-2](https://github.com/kaspanet/kips/blob/master/kip-0002.md) **Proposed**. [#1104](https://github.com/kaspanet/rusty-kaspa/pull/1104) open `a5888da`. [#1132](https://github.com/kaspanet/rusty-kaspa/pull/1132) open `703e1639` (parent-order gate). dagknight tip `ad45e24`. [#1135](https://github.com/kaspanet/rusty-kaspa/pull/1135) still **open** `ed09a8a` (not merged). No 100 BPS. No spendable L1 stable. |
...
| KCC still open | [#24](https://github.com/kaspanet/kccs/pull/24) `7159d48` KCC-0012. **22 Sep 09:44Z** Izio pointed at that pull and said KIP-12 is superseded by KCC-12 ([post](https://x.com/IzioDev/status/2102333259296145822)). The file still says `Status: Draft`, Backwards Compatibility says “No previous standard exists,” and the rationale calls KIP-12 a draft this KCC continues. No `kip-0012` in kaspanet/kips. Not a status change. He asked dapp and wallet authors for thoughts, and said not to send LLM thoughts. danieliyahu1 review [5778497208](https://github.com/kaspanet/kccs/pull/24#issuecomment-5778497208) (22 Sep 14:39Z): PSKB vectors use JSON numbers where §7.2 requires decimal strings, the single-signature rule blocks m-of-n co-signing, and §6.9 orders signatures by key bytes rather than redeem-script key order. Head still `7159d48`. Desk comment [5774902587](https://github.com/kaspanet/kccs/pull/24#issuecomment-5774902587): PSKB amounts are JSON numbers while Section 7.2 forbids that for uint64, and `kaspa_signTransaction` says display every output address when the output type only has `scriptPublicKey`. [#31](https://github.com/kaspanet/kccs/pull/31) `fb50affe` proposes KCC-20 `Status: Last Call` with no `Last-Call-Deadline`, rewrites `Created` from `2026-07-15` to `2026-08-21`, and leaves the README index at Draft. Not Last Call until that header exists and the pull merges. Desk comment [5775404197](https://github.com/kaspanet/kccs/pull/31#issuecomment-5775404197). **22 Sep 14:29 mail:** Manyfestation, on the new borrow paragraph in `kcc-0020.md`, said do not rephrase that restriction there. A valid borrow has more checks, and this is the wrong place. Remove the paragraph or point at the section that defines a valid borrow. The same review asks whether `Category` should be Asset. KCC-0 does not list Asset. Head still `fb50affe`. The deadline and the `Created` date are untouched. [#20](https://github.com/kaspanet/kccs/pull/20) vectors. [#23](https://github.com/kaspanet/kccs/pull/23) p2pk-ecdsa. [#30](https://github.com/kaspanet/kccs/pull/30) unkeyed hash. [#27](https://github.com/kaspanet/kccs/pull/27) `fa845057` approved, unmerged. [#26](https://github.com/kaspanet/kccs/pull/26) KCC-23 `479fa206`. [#6](https://github.com/kaspanet/kccs/pull/6) KCC-0021 `165ff91d` (touched 21 Sep 12:07Z). [#4](https://github.com/kaspanet/kccs/pull/4) KCC-0402. Issues, not PRs: #14 supply-split, #28 four “KCC20” objects. |
| KCC-3/4/5 | **Not on main.** [#29](https://github.com/kaspanet/kccs/pull/29) (`55742861`, kaspahttp402, 21 Sep 13:52Z) proposes Draft **Reputation Deed / Slashable Bond / Deed Identity Registry** as KCC 3, 4, and 5. Header author: Kaspa-World-Eater. They **Require** KCC-1 and KCC-2, which are still Draft. Numbers are a request, not an assignment. Workshop thread: [Kas-Smiths #148](https://kas-smiths.org/t/three-covenant-kccs-for-reputation-bonds-and-identity-live-on-testnet-10/148). Reference: [Kaspa-World-Eater/quorum](https://github.com/Kaspa-World-Eater/quorum). 21 Sep the README disagreed with itself. The author fixed both leftovers in [quorum@faa1a31](https://github.com/Kaspa-World-Eater/quorum/commit/faa1a31): the 21 tests stay chain-free, and the chain is one injected `SlashSubmitter`, described as proven once. Full id `81c3008f1fe5d79508105ada9b0760f4de52651de8b38a5039f549aeffaf172d`. api-tn10 accepted it, blue score 565268486, 1 KAS in, 0.99 KAS out. “Audit” is the author’s own pass. Desk confirm: [#29](https://github.com/kaspanet/kccs/pull/29#issuecomment-5768084395). Workshop [#148](https://kas-smiths.org/t/three-covenant-kccs-for-reputation-bonds-and-identity-live-on-testnet-10/148) still says this is before a pull request. Not adopted. Not a stable. |
| Kas-Smiths | [kas-smiths.org](https://kas-smiths.org). Workshop, opened 2 Jul 2026. Discourse 2026.8.0. Sole admin and moderator: Manyfest. Sign-in is X. Read 22 Sep: 44 topics, 356 posts, 111 users. Categories: General, Asset Standards, Product Use Cases, Meta. A thread is not a KIP. The law is the merged KCC file. Daily public mirror: [Manyfestation/kas-smiths-public-archive](https://github.com/Manyfestation/kas-smiths-public-archive), pushed 22 Sep 07:40Z. There is no GitHub user `kasmasmith` (July brand thread: pronounced KASMYTH). Still open on the workshop, with no reply: [#147](https://kas-smiths.org/t/wallet-integration-protocol/147) (daniel wants one wallet integration; the object is KCC-12 Draft, [#24](https://github.com/kaspanet/kccs/pull/24)) and [#141](https://kas-smiths.org/t/kcc-01-discussion-thread/141) (Ori, 29 Aug, on `int` / `bool` encoding and the array-of-records sentence in KCC-1). [#148](https://kas-smiths.org/t/three-covenant-kccs-for-reputation-bonds-and-identity-live-on-testnet-10/148) still says the three drafts are pre-PR. [#142](https://kas-smiths.org/t/wallet-utxo-covenant-id-lookup/142): Izio, covenant UTXO lookup waits on the UTXO-index storage rework. [#8](https://kas-smiths.org/t/fungible-token-covenant-specification-kcc20/8) last post 11 Sep: Manyfest said the KCC-20 spec is maturing toward finalization. The file is still Draft. |
| This desk | STP-KAS owns **52 public + 2 private** repos (21 Sep). Private: `tn10-grok`, `kns-kasware-tn10-test`. Every README was read: [`STP-REPOS.md`](STP-REPOS.md). All 54 have a disclaimer and the experimental banner. [kaspa-llm-forum](https://github.com/STP-KAS/kaspa-llm-forum) commit `fc746db` (21 Sep 09:04Z): **execution halted. Do not post reports. The clock does not run.** |
| vProgs | Master still [`f9b84a8`](https://github.com/kaspanet/vprogs/commit/f9b84a863a7c7c20586a9cf947550475e894f72e) (28 Jul). No release. **22 Sep ~08:32Z:** [#148](https://github.com/kaspanet/vprogs/pull/148) **closed unmerged** 07:30Z (closed head `78501881`; the old draft head was `da2a7f26`). Resume work is [#154](https://github.com/kaspanet/vprogs/pull/154), head `f84525b5`, 25 commits, base [#153](https://github.com/kaspanet/vprogs/pull/153). **22 Sep 10:53Z** Maxim marked #153 and #154 **draft**. Same heads. [#153](https://github.com/kaspanet/vprogs/pull/153) is one commit `3496a8c5` (cancel the lane-proof fetch on shutdown), base #147. [#152](https://github.com/kaspanet/vprogs/pull/152) is still **draft**: closed 07:29Z, reopened 07:30Z, head **`f61b46f4`** (22 Sep 12:58Z, `Merge settle-resume into reorg-safety`). It contains the gap retry `0309fd37` and #154 `f84525b5`. The earlier divergence is closed. Still draft. The demo tip `cc52b322` still pins `291d4f29` and does not contain this merge. [#156](https://github.com/kaspanet/vprogs/pull/156) does: its head is `30c5d021`. `74e24551...59b30920` **diverged**, then the demo lock caught up at 09:14Z (see tictactoe). **22 Sep 11:12Z:** [#156](https://github.com/kaspanet/vprogs/pull/156) draft, head `30c5d021`. [#158](https://github.com/kaspanet/vprogs/pull/158) draft `62882ad9` on `30c5d021` fixes [#112](https://github.com/kaspanet/vprogs/issues/112): restore was reading sub-base ids as orphaned, and a rollback to genesis skipped canonical filtering. [#157](https://github.com/kaspanet/vprogs/pull/157) was marked **draft** at 14:06Z. Head still `cd35f913`, base `30c5d021`: the bridge finalizes below the pruning point cited in headers after a confirmation window, instead of waiting for an IBD-only notification. Fixes vprogs#113. A malformed carrier input is journaled as a rejection instead of panicking the guest. Stack: #147 `74ee1b7d` → #153 → #154 → #152 → #156. Unchanged this morning: #149 `51b56aab`, #131 `004a5948`, #134 `add37bed`, #136 `d2da8ad0`, #140 `28f0aee9`. No other kaspanet PR updated on 22 Sep. [vprogs#114](https://github.com/kaspanet/vprogs/issues/114) is still open (body refreshed 22 Sep 14:17Z): a restored read-only batch can forward the disk value instead of the in-batch chain. No pull references it. The write-up caps it to non-proving and observer nodes. Research only. Desk comment [5774935036](https://github.com/kaspanet/vprogs/pull/152#issuecomment-5774935036): `reform_committed_gap` returns 0 on `LaneProofFailed` and does not run again, so a timeout is treated as a dead block for the life of the process. |
| tictactoe | Tip [`93b75901`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/93b75901e626) (22 Sep 13:33Z). `e2bc408` re-seeds the exit-index mirror from the store on a warm restart, so a family settled in the previous process still advances. Host and guest locks are still `kaspanet/vprogs` `guest-hardening#291d4f29`. [#156](https://github.com/kaspanet/vprogs/pull/156) head `30c5d021` also contains the gap retry. The demo pin does not. The fork pin `bridge-live-lane#128dd05f` is gone. Encoder wasm vendor is `0.1.6`. Rusty pin in the lock is still `eb0a856`, behind master `2a47b249` (#1096 and #1138). Not a product testnet. |
...
| Do not weld | Argent tag, KCC-20 Final, KCC-3/4/5 adopted, DAGKnight shipped, vProgs product, tictactoe = vProgs live, x402 mainnet, quorum = a convention, the kccs README “Draft” cell = KCC-0 still Last Call. |
...
| STP-KAS/kaspa.org-kaspaexplained | https://github.com/STP-KAS/kaspa.org-kaspaexplained | kaspa.org `/lore` still stale on Toccata. Do not cite lore for status. |
...
| [biryukovmaxim/vprog-tictactoe](https://github.com/biryukovmaxim/vprog-tictactoe) | **Guest demo, not a public product.** Tip `93b75901` (22 Sep 13:33Z) locks **`guest-hardening#291d4f29`**. [vprogs#152](https://github.com/kaspanet/vprogs/pull/152) HEAD is `0309fd37`. The guest fork pin is gone. [#24](https://github.com/biryukovmaxim/vprog-tictactoe/issues/24). |
