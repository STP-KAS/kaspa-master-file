...
| [kaspanet/vprogs#107](https://github.com/kaspanet/vprogs/issues/107) | **Open, no PR.** `AccessType::Read` still unenforced on `guest-hardening`: `data_mut`/`resize` set dirty; host `commit_changes` drops the write; lane wedges. Tictactoe `ResourceExt` papers over it; config `Init`/`Update` `write_new_state` still calls `data_mut` with no `is_writable`. Desk: [5788133052](https://github.com/kaspanet/vprogs/issues/107#issuecomment-5788133052). |
