...
| Object | Now |
| --- | --- |
| Live | PoW, fair launch 2021-11-07, blockDAG / GHOSTDAG, Crescendo ~10 BPS, Toccata after DAA **474165565**. KIP-16/17/20/21 **Active** ([kips `e4ae233`](https://github.com/kaspanet/kips/commit/e4ae2332117b5cb68bd6188e065ef885b6d17939)). rusty-kaspa release **[v2.0.1](https://github.com/kaspanet/rusty-kaspa/releases/tag/v2.0.1)** is commit **`cfafeb4c`** (15 Jun). Master [`ecb9b1f`](https://github.com/kaspanet/rusty-kaspa/commit/ecb9b1f5974b) (22 Sep 11:27Z, [#1096](https://github.com/kaspanet/rusty-kaspa/pull/1096) merged) is **not** that tag. The commit only fixes the output count inside `InvalidOutputIndex`. It is not a new consensus rule. Prior tip was `eb0a856`. SilverScript **[v1.0.0](https://github.com/kaspanet/silverscript/releases/tag/v1.0.0)** `3ed9733`. |
...
| vProgs | Master still [`f9b84a8`](https://github.com/kaspanet/vprogs/commit/f9b84a863a7c7c20586a9cf947550475e894f72e) (28 Jul). No release. **22 Sep ~08:32Z:** [#148](https://github.com/kaspanet/vprogs/pull/148) **closed unmerged** 07:30Z (closed head `78501881`; the old draft head was `da2a7f26`). Resume work is [#154](https://github.com/kaspanet/vprogs/pull/154), head `f84525b5`, 25 commits, base [#153](https://github.com/kaspanet/vprogs/pull/153). **22 Sep 10:53Z** Maxim marked #153 and #154 **draft**. Same heads. [#153](https://github.com/kaspanet/vprogs/pull/153) is one commit `3496a8c5` (cancel the lane-proof fetch on shutdown), base #147. [#152](https://github.com/kaspanet/vprogs/pull/152) is still **draft**: closed 07:29Z, reopened 07:30Z, head **`0309fd37`** (22 Sep 11:33Z, `zk: retry the committed-gap pass instead of abandoning it on a failed fetch`), one commit past `59b30920`. That is the retry in desk comment [5774935036](https://github.com/kaspanet/vprogs/pull/152#issuecomment-5774935036). `0309fd37` is ahead of `78501881` and **diverged** from #154 `f84525b5`. Same branch name `settle-resume` is not one commit stack. `74e24551...59b30920` **diverged**, then the demo lock caught up at 09:14Z (see tictactoe). **22 Sep 11:12Z:** [#156](https://github.com/kaspanet/vprogs/pull/156) draft on `reorg-safety` (`59b30920`), head `291d4f29` (22 Sep 11:44Z, guest ELF images refreshed for the rejection ABI). Prior head `58efe4a0`. It does not contain the #152 gap retry. A malformed carrier input is journaled as a rejection instead of panicking the guest. Stack: #147 `74ee1b7d` → #153 → #154 → #152 → #156. Unchanged this morning: #149 `51b56aab`, #131 `004a5948`, #134 `add37bed`, #136 `d2da8ad0`, #140 `28f0aee9`. No other kaspanet PR updated on 22 Sep. Research only. Desk comment [5774935036](https://github.com/kaspanet/vprogs/pull/152#issuecomment-5774935036): `reform_committed_gap` returns 0 on `LaneProofFailed` and does not run again, so a timeout is treated as a dead block for the life of the process. |
| tictactoe | Tip [`4f27dd1a`](https://github.com/biryukovmaxim/vprog-tictactoe/commit/4f27dd1a20ce) (22 Sep 09:14Z): `pin vprogs reorg-safety 59b30920`. **Cargo.lock read:** host `reorg-safety#59b30920`, which is no longer #152 HEAD (`0309fd37`). [#156](https://github.com/kaspanet/vprogs/pull/156) is still based on `59b30920`, so it does not contain the gap retry. `74e24551` is gone from that lock. Rusty pin in that lock is still `eb0a856`, one diagnostic commit behind master `ecb9b1f`. #152’s body still says the app pins `0636cff1` (the parent commit). **guest/Cargo.lock** was not in this commit; it is still the fork `bridge-live-lane#128dd05f`. [#25](https://github.com/biryukovmaxim/vprog-tictactoe/pull/25) left draft at 12:40Z. Head `76030511` (22 Sep 12:48Z, encoder wasm vendor `0.1.6` with the self-transfer guard). Host and guest locks both `kaspanet` `guest-hardening#291d4f29`. The fork `bridge-live-lane#128dd05f` pin is gone. Not merged. Master tip is still `4f27dd1a`. Not a product testnet. |
...
| rusty / DK / Argent / SS / vProgs | Tips: rusty master `ecb9b1f` (22 Sep, #1096). DK `ad45e24`, Argent `e76ee07`, SS `3ed9733`, vprogs `f9b84a8` hold. | No tip/tag/merge/Final. |
...
| vprogs | https://github.com/kaspanet/vprogs | Provable computation. Research. No public testnet as product. Master `f9b84a8` (28 Jul). **22 Sep:** [#148](https://github.com/kaspanet/vprogs/pull/148) closed unmerged. Stack #147 → [#153](https://github.com/kaspanet/vprogs/pull/153) → [#154](https://github.com/kaspanet/vprogs/pull/154) draft `f84525b5` → [#152](https://github.com/kaspanet/vprogs/pull/152) draft `0309fd37` (gap retry; diverged from #154). Guest-demo lock still `59b30920`. |
...
| [biryukovmaxim/vprog-tictactoe](https://github.com/biryukovmaxim/vprog-tictactoe) | **Guest demo, not a public product.** Tip `4f27dd1a` (22 Sep 09:14Z) locks **`reorg-safety#59b30920`**. [vprogs#152](https://github.com/kaspanet/vprogs/pull/152) HEAD is now `0309fd37`. Guest ELF still fork `bridge-live-lane#128dd05f`. [#24](https://github.com/biryukovmaxim/vprog-tictactoe/issues/24). |
